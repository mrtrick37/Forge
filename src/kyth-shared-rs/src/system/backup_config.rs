...
impl Default for BackupConfig {
    fn default() -> Self {
        Self {
            repo: "/var/cache/kyth/backup".into(),
            btrfs_send: false,
            on_battery: false,
            remote: String::new(),
        }
...
pub fn load(path: impl AsRef<Path>) -> BackupConfig {
    ...
    BackupConfig {
        repo: table
            ...
            .unwrap_or("/var/cache/kyth/backup")
            .to_string(),
        btrfs_send: table
            .and_then(|table| table.get("btrfs_send"))
            .and_then(toml::Value::as_bool)
            .unwrap_or(false),
        on_battery: table
            .and_then(|table| table.get("on_battery"))
            .and_then(toml::Value::as_bool)
            .unwrap_or(false),
        remote: table
