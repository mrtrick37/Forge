...
pub fn toml_scalar(value: &toml::Value) -> String {
    match value {
        ...
        toml::Value::Array(items) => {
            format!(
                "[{}]",
                items.iter().map(toml_repr).collect::<Vec<_>>().join(", ")
            )
        }
...
pub fn apply_sections(
    ...
    for (section, entries) in sections {
        let Some((file, groups)) = parse_section(section) else {
            continue;
        };
...
mod tests {
    ...
    fn flattens_nested_tables_like_python() {
        ...
        assert_eq!(
            sections["kwinrc.Compositing"].get("AllowTearing"),
            Some(&"false".to_string())
        );
        assert_eq!(
            sections["kwinrc.Containments.1.General"].get("foo"),
            Some(&"bar".to_string())
        );
    }
    ...
    fn parses_sections_with_general_default() {
        assert_eq!(
            parse_section("kwinrc"),
            Some(("kwinrc".to_string(), vec!["General".to_string()]))
        );
        assert_eq!(
    ...
    fn applies_sections_and_skips_invalid_ones() {
        ...
        sections.insert(
            "kwinrc.Compositing".to_string(),
            BTreeMap::from([("AllowTearing".to_string(), "false".to_string())]),
        );
        sections.insert(
            "...".to_string(),
        ...
        assert_eq!(
            applied,
            vec!["kwinrc.Compositing:AllowTearing=false".to_string()]
        );
