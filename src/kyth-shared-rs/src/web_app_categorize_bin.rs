...
use kyth_shared::system::desktop_shortcuts::{
    WEB_APP_GLOBS, categorize_web_app, matches_web_app_name,
};
...
fn is_watched(name: &str) -> bool {
    WEB_APP_GLOBS.iter().any(|pattern| matches_web_app_name(name, pattern))
}
...
fn categorize() -> bool {
    ...
    for path in &entries {
        ...
        let Some(updated) = categorize_web_app(&content) else { continue };
