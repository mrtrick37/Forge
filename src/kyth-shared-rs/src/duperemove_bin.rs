...
use kyth_shared::system::maintenance::{
    dedupe_command, find_dedupe_targets, secure_dedupe_hash_file, supports_dedupe_filesystem,
    DUPE_HASH_DIR,
};
...
fn main() -> std::process::ExitCode {
    ...
    for target in &targets {
        ...
        if !supported {
            log(&format!(
                "skipping unsupported filesystem for {}",
                target.display()
            ));
            continue;
        }
        log(&format!("deduping {}", target.display()));
        let Ok(hash_file) = secure_dedupe_hash_file(target, &state_dir) else {
            continue;
        };
