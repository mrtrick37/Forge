...
use kyth_shared::system::vrr::{
    TTL_PATH, TTL_SECS, config_path, doctor_mode, global_policy, is_output_name_valid, kwin_argv, load,
    mode_for_policy, per_output_argv,
};
...
fn write_key(binary: &str, group: &str, key: &str, value: &str, value_type: Option<&str>) -> bool {
    run_bounded(&kwin_argv(binary, group, key, value, value_type), Duration::from_secs(5))
...
fn main() -> std::process::ExitCode {
    ...
    if let Some(binary) = first_binary(&["kwriteconfig6", "kwriteconfig5", "kwriteconfig"]) {
        let policy = if config.outputs.is_empty() { "1" } else { global_policy(&config.outputs) };
        if write_key(&binary, "Wayland", "VrrPolicy", policy, None) {
            applied.push(format!("Wayland.VrrPolicy={policy} ({})", mode_for_policy(policy)));
        }
        if write_key(&binary, "NightColor", "Active", &config.night_enabled.to_string().to_lowercase(), Some("bool")) {
            applied.push(format!("NightColor.Active={}", py_bool(config.night_enabled)));
        }
        if write_key(&binary, "NightColor", "Mode", "2", None) {
            applied.push("NightColor.Mode=2".to_string());
        }
        if write_key(&binary, "NightColor", "NightTemperature", &config.night_temperature.to_string(), None) {
            applied.push(format!("NightColor.NightTemperature={}", config.night_temperature));
        }
    }
    ...
    let notes = applied;
    println!("kyth-apply-vrr: {}", if notes.is_empty() { "nothing to apply".to_string() } else { notes.join("; ") });
